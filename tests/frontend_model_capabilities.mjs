...
assert.equal(api.reasoningLabel(api.descriptor({ reasoning: true })), '支持（Hermes 模型目录声明） · 强度：未公开', 'boolean support must not be presented as a fake Auto effort');
assert.equal(api.reasoningLabel(api.descriptor({ reasoning: false })), '不支持（Hermes 模型目录声明） · 强度：不适用');
assert.equal(api.reasoningLabel(api.descriptor({})), '未确认（Hermes 未声明） · 强度：未确认', 'missing capability must remain visibly unconfirmed');
assert.match(api.reasoningLabel(api.descriptor({ reasoning: true, can_disable_reasoning: true })), /支持.*强度：未公开.*可关闭/);
