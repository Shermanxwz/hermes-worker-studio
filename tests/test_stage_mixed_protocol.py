...
class MixedProtocolStageTransformTests(unittest.TestCase):
    ...
    def test_backend_transform_is_syntax_valid_and_contains_lazy_resolver(self):
        ...
        for token in (
            ...
            "route = _resolve_route_for_run(provider, model)",
    ...
    def test_frontend_transform_routes_chat_worker_verifier_through_one_resolver(self):
        for token in (
    ...
    def test_frontend_transform_is_javascript_syntax_valid(self):
        node = shutil.which("node")
        if not node:
            self.skipTest("node is unavailable")
        with tempfile.TemporaryDirectory() as directory:
            target = pathlib.Path(directory) / "index-v3.js"
            target.write_text(self.frontend, encoding="utf-8")
